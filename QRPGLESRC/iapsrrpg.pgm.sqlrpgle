...
//        |        |            | (Task - #689)                                         //
//27/01/25| 0025   |Vamsi       | Removal of comments in RPG3 is updated based on source//
//        |        |Krishna2    | specification (Task#1127)                             //
//23/01/25| 0026   |Vamsi       | IAPGMFILES is restructured.So,updated the columns     //
//        |        |Krishna2    | accordingly.(Task#63)                                 //
//01/08/24| 0027   | Sabarish   | IFS Member Parsing Feature                            //
//------------------------------------------------------------------------------------- //
...
dcl-s r_sqlstm        char(2000)   inz;                                                  //0027
...
Begsr Format_String;
   ...
   select;
      ...
      when  r_Src_Type = 'FX3' and r_fix = 'C' and %subst(r_stm:60) <> *blanks;          //0028
         r_stm = %replace(' ':r_stm:60:33);                                              //0028

      when  r_Src_Type = 'FX3' and r_fix = 'E' and %subst(r_stm:58) <> *blanks;          //0028
         r_stm = %replace(' ':r_stm:58:35);                                              //0028

      when  r_Src_Type = 'FX3' and r_fix = 'H' and %subst(r_stm:81) <> *blanks;          //0028
         r_stm = %replace(' ':r_stm:81:12);                                              //0028

      when  r_Src_Type = 'FX3' and %subst(r_stm:75) <> *blanks;                          //0028
         r_stm = %replace(' ':r_stm:75:18);                                              //0028
...
begsr Process_SQL;
   ...
   IAPSRSQL(r_string        : r_type          : r_error         :
         // uwxref          : UwSrcDtl.srclib : UwSrcDtl.srcspf :                        //0027
         // UwSrcDtl.srcmbr : r_rrns          : r_rrne          :                        //0027
            uwxref :UwSrcDtl: r_rrns          : r_rrne          :                        //0027
   ...
   when %scan('END-DS' : %trim(r_string)) = 1;
      ...
      if r_dsdcl = 'DS';
         ...
         IAPSRDSFR(r_string        : r_type          : r_error         : uwxref   :
          //0027   UwSrcDtl.srclib : UwSrcDtl.srcspf : UwSrcDtl.srcmbr : r_rrns   :
                   UwSrcDtl : r_rrns   :                                             //0027
         ...
         if r_dsdcl = 'DS';
            ...
            if r_string <> *blanks and %scan('DCL-DS ' : %trim(r_string)) = 0;
               ...
               IAPSRVARFR(r_string        : r_type          : r_error      :
                        // 0027  uwxref          : UwSrcDtl.srclib : UwSrcDtl.srcspf :
                        // 0027  UwSrcDtl.srcmbr : r_rrns          : r_rrne          :
                          uwxref          : UwSrcDtl : r_rrns : r_rrne       :           //0027
                          in_procedure                                       );
            endif;
...
begsr FindDsDtl;
   ...
   exec sql                                                                              //0026
     select iAActFile,iAFilInfDs,iAIndDs  into :uwdstyp                                  //0026
       from iAPgmFiles                                                                   //0026
     where iALibNam   = trim(:uwSrcDtl.SrcLib)                                           //0026
       and iASrcFile  = trim(:uwSrcDtl.SrcSpf)                                           //0026
       and iAMbrName  = trim(:uwSrcDtl.SrcMbr)                                           //0026
       and (iAFilInfDs  = trim(:r_DsName)                                                //0026
       or   iAIndDs  = trim(:r_DsName) )                                                 //0026
     fetch first row only;                                                               //0026
...
begsr Process_PRPI;
   ...
   select;
      when %scan('DCL-PR ':r_string:1) > 0
         ...
         //    WriteProceduredetail( UwSrcDtl.srcmbr                                  0027 //0011
         //                         :UwSrcDtl.srcspf                                  0027 //0011
         //                         :UwSrcDtl.srclib                                  0027 //0011
               WriteProceduredetail( UwSrcDtl                                              //0027
         ...
         select;
            ...
            when %scan('END-PR ': r_string ) > 0
               ...
               // 0027 IAPSRPRFR(r_string        : r_type          : r_error         :
               // 0027          uwxref          : UwSrcDtl.srclib : UwSrcDtl.srcspf :
               // 0027          UwSrcDtl.srcmbr : r_rrns          : r_rrne          :
               // 0027          r_srcSeqs       : in_procedure    : *omit          );     //0019

               IAPSRPRFR(r_string        : r_type          : r_error         :             //0027
                         uwxref          : UwSrcDtl        : r_rrns          :             //0027
                         r_rrne          : r_srcSeqs       : in_procedure    :             //0027
                         *omit          );                                                 //0027

              // Passing the procedure name in case of PI                                  //0019
              when  %scan('END-PI ': r_string ) > 0                                        //0019
                   or %scan('END-PI;': r_string ) > 0;                                     //0019
               // 0027  IAPSRPRFR(r_string        : r_type          : r_error         :    //0019
               // 0027            uwxref          : UwSrcDtl.srclib : UwSrcDtl.srcspf :    //0019
               // 0027            UwSrcDtl.srcmbr : r_rrns          : r_rrne          :    //0019
               // 0027            r_srcSeqs       : in_procedure : procedureNm        );   //0019

                 IAPSRPRFR(r_string        : r_type          : r_error         :           //0027
                           uwxref          : UwSrcDtl        : r_rrns          :           //0027
                           r_rrne          : r_srcSeqs       : in_procedure    :           //0027
                           procedureNm        );                                           //0027
                 clear procedureNm;                                                        //0027
      ...
      when r_fix = 'P' and %subst(r_string:24:1) = 'B';
        ...
        enddo;
         ...
         IAPSRPRFX(r_prstring      : r_type          : r_error         : uwxref :            //SG01
                   //0027 UwSrcDtl.srclib : UwSrcDtl.srcspf : UwSrcDtl.srcmbr : r_rrns :
                   //0027 r_rrne          : r_srcSeqs       : r_longprname :
                   UwSrcDtl : r_rrns : r_rrne : r_srcSeqs : r_longprname :                   //0027
                   in_procedure);                                                        //0020
                                                                                             //SG01
         //Remove                                                                           //SG01
    ...
    //Exec Sql Fetch SrcStmt Into :R_Stm, :R_Rrn;                                        //0012
     ...
     Dow (%SubSt(r_String:6:1) = 'C' And                                                 //0008
        ...
        IAPSRPLSTFX(R_string        : R_type          : R_error         : Uwxref         //0008
           //0027   : UwSrcDtl.Srclib : UwSrcDtl.Srcspf : UwSrcDtl.Srcmbr : R_Rrns       //0008
                    : UwSrcDtl : R_Rrns                                                  //0027
            ...
            if %scan(%trim(r_Procname):r_string) > 0                                     //0013
               ...
               if %scan(' CALLP':r_string) > 0;                                               //BS02
                  IAPSRCALLP(r_string        : r_type          : r_error         :            //BS02
                           // 0027  uwxref          : UwSrcDtl.srclib : UwSrcDtl.srcspf :     //BS02
                           // 0027  UwSrcDtl.srcmbr : r_rrns          : r_rrne );             //BS02
                           uwxref          : UwSrcDtl  : r_rrns          : r_rrne );          //0027
               endif;                                                                         //BS02
...
//----------------------------------------------------------------------------------
   r_sqlstm = 'select  SOURCE_RRN, SOURCE_SEQ, SRCLIN_TYPE, SRCLIN_CNTN,'+               //0027
              'Upper(SOURCE_DATA) from IaQrpgsrc WHERE '+                                //0027
              'substring(SRCLIN_TYPE,4,1)= '' '' and SRCLIN_TYPE <> '' '' AND';          //0027

   If UwSrcDtl.ifsloc <> *blanks;                                                        //0027
     r_sqlstm = %trim(r_sqlstm) + ' IFS_LOCATION = ''' + %trim(UwSrcDtl.ifsloc)+         //0027
                             ''' AND MEMBER_NAME = ''' + %trim(UwSrcDtl.srcmbr)+         //0027
                             '''order by SOURCE_rrn';                                    //0027
   else;                                                                                 //0027
     r_sqlstm = %trim(r_sqlstm) + ' LIBRARY_NAME = ''' + %trim(UwSrcDtl.srclib)+         //0027
                           ''' AND SOURCEPF_NAME = ''' + %trim(UwSrcDtl.srcspf)+         //0027
                           ''' and MEMBER_NAME   = ''' + %trim(UwSrcDtl.srcmbr)+         //0027
                           ''' order by SOURCE_rrn';                                     //0027
   endif;                                                                                //0027

   exec sql prepare Stmt from :r_sqlstm;                                                 //0027
   exec sql declare SrcStmt cursor for stmt;                                             //0027
   //0027 exec sql
   //0027   declare SrcStmt cursor for
   //0027  //select Upper(SOURCE_DATA) ,SOURCE_RRN, SRCLIN_TYPE                                 //00
   //0027    select  SOURCE_RRN, SOURCE_SEQ, SRCLIN_TYPE, SRCLIN_CNTN,                          //00
   //0027            Upper(SOURCE_DATA)                                                         //00
   //0027    from IaQrpgsrc                                                                     //00
   //0027    where LIBRARY_NAME = trim(:UwSrcDtl.srclib)                                        //00
   //0027     and SOURCEPF_NAME = trim(:UwSrcDtl.srcspf)                                        //00
   //0027     and MEMBER_NAME   = trim(:UwSrcDtl.srcmbr)                                        //00

   //0027     and SRCLIN_TYPE <> 'FFRC' and SRCLIN_TYPE <> 'FFCC'  and                          //00
   //0027     SRCLIN_TYPE <> 'FX3C' and SRCLIN_TYPE <> 'FX4C' and                               //00
   //0027     SRCLIN_TYPE <> ' '                                                                //00
   //0027     Order by SOURCE_rrn;
    ...
    //exec sql fetch SrcStmt into :r_stm , :r_rrn, :r_Src_Type;                          //0007
      ...
      dow sqlCode = successCode;
        ...
        //                                       :UwSrcDtl.mbrtyp1,                  //0010 //BA06
            ...
            // IAPSROSPEC(r_string: r_type : r_error : uwxref :UwSrcDtl.srclib:          //0009 0027
            //            UwSrcDtl.srcspf: UwSrcDtl.srcmbr : r_rrns: r_rrne);            //0009 0027
               IAPSROSPEC(r_string :r_type  : r_error : uwxref :                         //0027
                          UwSrcDtl : r_rrns : r_rrne);                                   //0027
                                                                                         //0009
            endif;                                                                       //0009
